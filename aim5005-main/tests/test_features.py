...
class TestFeatures(TestCase):
    ...
    def test_standard_scaler_transform(self):
        ...
        scaler.fit(data)
        result = scaler.transform(data)  # Call transform and store the result
    ...
    def test_label_encoder_numeric(self):
        ...
        # Test fit
        encoder.fit(data)
        assert np.array_equal(encoder.classes_, [10, 20, 30]), "Classes are incorrect for numeric data"

        # Test transform
        encoded = encoder.transform(data)
        assert np.array_equal(encoded, [0, 1, 0, 2]), "Encoded numeric labels are incorrect"

        # Test fit_transform
        fit_encoded = encoder.fit_transform(data)
        assert np.array_equal(fit_encoded, [0, 1, 0, 2]), "Fit_transform numeric labels are incorrect"

        # Test with new numeric data
        new_data = [20, 30]
        new_encoded = encoder.transform(new_data)
        assert np.array_equal(new_encoded, [1, 2]), "New numeric data encoding is incorrect"

        print("LabelEncoder numeric test passed!")
